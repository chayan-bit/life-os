...
**Implemented (issue #137):** `server/build/t5Subsystem.js`'s `buildT5Node(ctx, node)` runs the four roles in sequence over ONE shared worktree + the T5 Layer B/C sandbox, each role exactly one injectable `ctx.queryFn` call (the same DI seam as every other tier), via the shared consumption contract in the new `server/build/buildAgent.js` (`consumeRole`, the tool allowlists, `BuildNodeSummary`, `trackedHook`, `nodeKey` - extracted from `node.js` so the two build paths define them once). **Supervisor** and **reviewer** are read-only structured calls (`allowedTools: ["Read","Glob","Grep"]`, no write tools, no hook needed) - the supervisor returns a file-level plan `{ files:[{path,purpose}], testStrategy, riskNotes }` (persisted as a `build.t5.planned` event, the append-only trace), the reviewer an adversarial verdict `{ approve, issues }`. **Scaffolder** and **tester** are tool-using calls with the full allow/disallow set + the tracked PreToolUse hook + Seatbelt sandbox, identical to a T1-T4 build node; the tester's prompt confines it to tests-only against a `std::env::temp_dir()` scratch DB (never `lifeos.db`), mirroring T3's convention. A reviewer rejection triggers ONE bounded fix round (scaffolder re-invoked with the issues, then a re-review); a second rejection throws and the node fails - no endless loop. Two back-pressure budgets bound the split (§11, [AGENT-CORE.md](./AGENT-CORE.md) §3): a role-call cap `MAX_T5_QUERY_CALLS = 6` (supervisor+scaffolder+tester+reviewer+fix+re-review) and a per-node token ceiling (`ctx.budget` or `T5_TOKEN_BUDGET`), summed across roles from each result's SDK `usage` and checked BEFORE every call - a run that would breach either stops and fails with `budget_exhausted`, the same fail-closed discipline as the agent gate. The reviewer's verdict is threaded on the built node (`built.review`) to the T5 validator (§9), which independently re-runs cargo and refuses a node without a recorded `{ approve: true }`. This is the bounded supervisor+3, **not** a 12-path swarm; a genuine multi-node build DAG (e.g. a finance request emitting T0+T1+T4 plus this T5 crate) is still the pipeline's job (§4) - T5's split is the multi-agent build of the single crate node, not a replacement for the DAG.
...
**Implemented (issue #137) - typed-confirm flag for T5:** T5 is the
highest-blast-radius tier (a whole crate), so `server/build/gate.js` now stamps
`requires_typed_confirm: node.tier === "T5"` onto the `pending_approval` entity's
attrs (in addition to the normal T3+ halt). The downstream approval UI/bot is
expected to demand an EXPLICIT typed confirmation for a flagged node rather than
a one-tap approve; that consumer is out of scope here - the flag on the entity is
the deliverable and the contract. T3/T4 approvals are unchanged (flag `false`).
...
**Implemented (issue #136):** T4 is the fourth tier past T0 to get a real generator + validator - an **additive migration**, the first tier whose blast radius is schema itself rather than code. `server/lib/tierScopes.js`'s `TIER_SCOPES.T4` is `({ name }) => [\`migrations/*_${name}.sql\`]` - the concrete `<NNNN>` prefix can't be resolved inside a static glob (it is "current max + 1" at build time), so the scope is a single-file wildcard on the number and the validator (not the glob) enforces "exactly one new file, correctly numbered"; the original design note's second half, `services/lifeos-derived/**`, is dropped - that crate/DB does not exist in this repo, the derived DB's DDL is baked into `lifeos-api/src/db.rs` as `include_str!` constants applied to a physically separate file, not read generically from `migrations/` (docs/DATA-MODEL.md §7's issue #136 note has the full honesty statement). `server/build/plan.js`'s `REQUIRED_PARAMS.T4` is now `["name"]` (previously `[]`) since the write-scope needs it. `server/build/node.js`'s `TIER_PROMPTS.T4` is a real prompt: author exactly one `migrations/<NNNN>_<name>.sql` file, restricted to `GENERATED ALWAYS AS (...) VIRTUAL` columns, plain nullable `ADD COLUMN`, `CREATE INDEX IF NOT EXISTS` (incl. expression indexes), and `CREATE VIRTUAL TABLE IF NOT EXISTS ... fts5(...)` - forbidding DROP/UPDATE/DELETE/RENAME/CREATE TRIGGER/non-virtual CREATE TABLE outright and naming the memvec/lexical-index tables that must never be targeted. `server/validators/t4Migration.js` (`getValidators('T4')` now returns `[protectedSurfaceValidator, t4MigrationValidator]`, no longer the placeholder) is the build-time gate: a file-discipline check (`git status --porcelain`, exactly one new file, numbered current-max + 1 - git status/diff always shell the real `git`, never mocked, mirroring t3Route's DI boundary), a statement classifier (semicolon-aware, comment-tolerant split; an explicit destructive denylist - DROP/DELETE/UPDATE/TRUNCATE/RENAME/CREATE TRIGGER - wins before any allowed-shape match is attempted; fail-closed on anything unrecognized), a derived-ownership text scan (rejects a migration naming `entity_vec`/`entity_vec_meta`/`llm_cache`/`entities_idx`/`entities_fts`/`memory_idx`/`memory_fts` outright), and finally a scratch-apply no-rewrite proof: every existing migration is applied in numeric order to a fresh scratch DB via the real `sqlite3` CLI (shelled through the injectable `opts.execFn`, the same DI seam T3 established), `sqlite_master` + `PRAGMA table_info` are snapshotted, the new migration is applied, and a pure `diffSnapshots` function (fixture-tested independent of any sqlite exec) asserts no pre-existing column or non-table schema object changed - deliberately excluding a table's own `sqlite_master` text from that check, since a legitimate `ADD COLUMN` always rewrites SQLite's recorded `CREATE TABLE` text and that is not the harmful rewrite this validator exists to catch; when every statement in the file is idempotent (`CREATE ... IF NOT EXISTS` shapes only - a plain `ADD COLUMN` is not, matching the `add_column_if_missing` guard's own reasoning in `lifeos-api/src/db.rs`), the migration is applied a second time to prove that claim too. `server/test/buildPipeline.test.js`'s pre-existing "T4 validator placeholder fails closed" test is retargeted to T5 (the last remaining placeholder) and a new test exercises the REAL `t4Migration` validator end to end (real `sqlite3`, no execFn mock needed since it is fast) through to the `awaiting_approval` gate halt, unchanged from T3's approval-only gate boundary (`GATED_TIERS` in `server/build/plan.js` already included T4 from issue #132's original design). **Known limitation, honestly documented:** a T4 migration that lands is not yet wired into `lifeos-api/src/db.rs`'s `run_migrations()` - the migration list there is a hardcoded, ordered sequence of `include_str!` constants (docs/DATA-MODEL.md §7's note), so a shipped T4 file takes effect at runtime only once a human adds its constant + call, exactly like every prior migration in this repo's history; automating that wiring safely (or replacing it with a generic migration-runner) is a follow-up, not invented here.

**Implemented (issue #137):** T5 is the top rung and the LAST placeholder to fall - a whole new **crate/subsystem**, built by the bounded supervisor + subagent split (§7) rather than a single call. `server/lib/tierScopes.js`'s `TIER_SCOPES.T5` is `({ crate }) => [\`services/${crate}/**\`, "services/Cargo.toml"]` (unchanged from #131 - verified) and `server/build/plan.js`'s `REQUIRED_PARAMS.T5` is `["crate"]` (unchanged). `server/build/node.js`'s `buildNode` now dispatches a T5 node to `server/build/t5Subsystem.js`'s `buildT5Node` (T0 still delegates to `scaffoldModule`, T1-T4 unchanged through `buildTierNode`); the four shared build-agent primitives (`consumeRole`, the tool allowlists, `BuildNodeSummary`, `buildNodeSummaryJsonSchema`, `trackedHook`, `nodeKey`) moved to the new `server/build/buildAgent.js` so both build paths reuse one definition, and `TIER_PROMPTS.T5` (the old generic single-call template) is gone - T5's prompts are one per role in `t5Subsystem.js`. `server/validators/t5Crate.js` (`getValidators('T5')` now returns `[protectedSurfaceValidator, t5CrateValidator]`, no longer the placeholder) is the build-time gate: a scope check (every changed path confined to `services/<crate>/**` or the one shared `services/Cargo.toml` - git `status --porcelain` is the build's full diff since nothing is committed yet at validate time, git always shelled real, never mocked), an additive-only `services/Cargo.toml` diff check (the workspace `members` array may only gain a line - a pure-addition unified diff never emits a `-` body line, the same proof T3 applies to `mod.rs`), a reviewer-sign-off check (the pipeline threads `built.review` through `server/build/validate.js`'s new `VALIDATOR_ARGS.t5Crate` arg builder; a node without a recorded `{ approve: true }` fails - belt-and-braces over `t5Subsystem.js`'s own in-loop enforcement), and finally `cargo build -p <crate>`, `cargo test -p <crate>`, and `cargo clippy -p <crate> -- -D warnings` shelled through the injectable `opts.execFn` (threaded from `runBuildPipeline`'s `opts.execFn`), so vitest never runs real cargo. **Eval-gate boundary, honestly scoped:** identical to T3/T4 - the sampled Haiku judge has no callable surface from this JS pipeline (§8's #132 note), so a validated T5 node flows through `gate.js`'s approval-only halt, now stamped `requires_typed_confirm: true` (§8), and is never auto-committed. `server/test/buildPipeline.test.js`'s pre-existing "T5 validator placeholder fails closed" test is REPLACED by the real path (supervisor->scaffolder->tester->reviewer through the REAL `t5Crate` validator with cargo mocked, to the `awaiting_approval` + `requires_typed_confirm` gate halt) plus tests for a failing-cargo-test block, a one-round reviewer-reject-then-approve recovery, a double-reject node failure (no third round), and a `budget_exhausted` stop; a new registry test asserts every tier T0-T5 now resolves to real validators with NO `notImplemented` placeholder for any known tier (the placeholder survives only for an unknown tier). `server/test/t5Crate.test.js` unit-tests the validator directly against a real scratch git repo (scope, additive-Cargo.toml, sign-off, and cargo-DI failure paths). **Known limitation, honestly documented:** no real Rust crate is generated by this issue - the machinery (roles, budgets, validator, gate flag) is the deliverable, exercised end to end with mocked SDK + cargo; a first real T5 crate, and wiring a shipped crate's `members` entry into a live `cargo` workspace build on the trusted Mac, follow when a concrete subsystem request lands.
