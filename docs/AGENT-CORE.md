...
- **Self-eval regression suite.** Golden tool-routing scenarios (given a request, does the loop reach a sensible tool and avoid a wrong one?) run side-effect-free, so self-evolution and prompt/manual changes can't silently regress routing. Complements the quality-focused [eval-gate](./HARNESS-LOOP.md) §2. **Implemented (issue #140):** a `ctx.dryRun` flag threaded through `server/agent/{executor,gate,loop}.js` makes a turn fully side-effect-free - every tool call still lands in the ledger with a synthetic `{status:'dry_run'}` result (gated tools recorded-not-enqueued, forbidden calls recorded-not-denial-logged) while every write (`persistTurn`, plan persistence, Tool-RAG indexing, memory ingest, lesson distillation, gate escalations, the LLM cache) is skipped; `server/evals/scenarios.json` holds 8 golden routing scenarios (`{id, prompt, expect_any, forbid}`, zod-validated at load) and `server/evals/run.js` runs each through the real `runAgentTurn` in dry-run mode, scores expect_any-hit-and-no-forbid, appends a `{ts, total, passed, rate, failures}` line to the gitignored `server/evals/history.jsonl`, and exits non-zero below `PASS_THRESHOLD` (0.8) - wired as an optional CI/self-evolution gate. Tests: `server/test/evals.test.js`.
...
- **Frontend:** `AIConsole.jsx` is migrated onto `/api/agent` (issue #122 follow-up): the console `POST`s the prompt (+ `workspace_id`) to the loop and renders its final text, executed steps (`ledger`) and gated `pendingApprovals` drafts. The bespoke `actionPlanCompiler.js` / `ActionPlanPreview.jsx` prompt-and-parse path was removed. Browser-only navigation stays client-side through the action registry; a fast `capabilities.js` guardrail pre-filter short-circuits obviously-forbidden asks, with the loop's gate as the authority. Applied mutations render in the Agent Ledger, each undoable.
- **Must-pass checks:**
